...
const DOT_DELAYS = ["0s", "0.1s", "0.2s", "0.7s", "0.3s", "0.6s", "0.4s", "0.5s", "0.0s"] as const;
  ...
  function Spinner({ className, ...rest }, ref) {
    return (
      <span
        ...
        role="status"
        aria-label="Loading"
        className={cn("inline-grid h-6 w-6 grid-cols-3 grid-rows-3 gap-[2px]", className)}
        {...rest}
      >
        {DOT_DELAYS.map((delay, i) => (
          <i
            key={i}
            className={cn("block h-1 w-1", i === 4 ? "bg-accent" : "bg-fg-muted")}
            style={{
